...
VECTOR_STORE_PATH = "persistent_faiss_llaama3_2"
...
if 'llama3.2:latest' not in df.columns:
    df['llama3.2:latest'] = None
...
for question, ai_response in results:
    df.loc[df['Question'] == question, 'llama3.2:latest'] = ai_response

# Save to CSV
df.to_csv('llama3.2:latest.csv', index=False)

print("✅ All answers saved to 'llama3.2:latest.csv'")
